...
class InMemoryProvider extends Provider with LazyLogging {
  ...
  private case class KeyContents(lastModificationTime: DateTime,
                                 data: Array[Byte])
  ...
  override def listBuckets: ListAllMyBuckets = {
    val buckets = bucketDataStore map {
      case (name, data) => Bucket(name, data.creationTime)
    }
    logger.debug(s"listing buckets: ${buckets.map(_.name)}")
    ListAllMyBuckets("root", UUID.randomUUID().toString, buckets.toList)
  }

  override def listBucket(bucket: String,
                          prefix: Option[String],
                          delimiter: Option[String],
                          maxkeys: Option[Int]): ListBucket = {
    ...
    bucketDataStore.get(bucket) match {
      case Some(bucketContent) =>
        ...
        val matchResults = matchingKeys map {
          case (name, content) =>
            Content(
              name,
  ...
  override def putObject(bucket: String,
                         key: String,
                         data: Array[Byte],
                         objectMetadata: ObjectMetadata): Unit = {
    bucketDataStore.get(bucket) match {
      case Some(bucketContent) =>
        logger.debug(
  ...
  override def putObjectMultipartPart(bucket: String,
                                      key: String,
                                      partNumber: Int,
                                      uploadId: String,
    ...
    bucketDataStore.get(bucket) match {
      case Some(bucketContent) =>
        val completeBytes = multipartTempStore(uploadId).toSeq
          .map(_.data)
          .fold(Array[Byte]())(_ ++ _)
        bucketContent.keysInBucket
          .put(key, KeyContents(DateTime.now, completeBytes))
      ...
      case (Some(srcBucketContent), Some(dstBucketContent)) =>
        srcBucketContent.keysInBucket.get(sourceKey) match {
          case Some(srcKeyContent) =>
            ...
            sourceMeta.foreach(
              meta => metadataStore.put(destBucket, destKey, meta)
            )
            CopyObjectResult(
              destFileModTime,
