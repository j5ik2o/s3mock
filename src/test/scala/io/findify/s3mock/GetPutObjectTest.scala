  ...
  override def behaviour(fixture: => Fixture): Unit = {
    ...
    it should "store tags and spit them back on get tagging requests" in {
      ...
      var tagging = s3
        ...
        .getTagSet
        .asScala
      var tagMap = new util.HashMap[String, String]().asScala
      for (tag <- tagging) {
        tagMap.put(tag.getKey, tag.getValue)
      }
      tagMap.size shouldBe 2
      tagMap.get("key1") shouldBe Some("val1")
      tagMap.get("key=&interesting") shouldBe Some("value=something&stragne")
    }
